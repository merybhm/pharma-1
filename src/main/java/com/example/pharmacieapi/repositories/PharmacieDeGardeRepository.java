...
public interface PharmacieDeGardeRepository extends JpaRepository<PharmacieDeGarde, PharmacieDeGardeRelation> {
    ...
    @Query("select pg from PharmacieDeGarde pg where pg.garde.idGarde =:id and CURRENT_DATE BETWEEN pg.PharmacieDeGardeRelation.dateDebut and pg.dateFin")
    List<PharmacieDeGarde> findAllPharmacieGardeByType(@Param("id") int id);
